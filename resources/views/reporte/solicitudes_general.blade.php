            ...
            <tbody>
                @forelse($solicitudes as $solicitud)
                    <tr>
                        ...
                        <td class="text-bold">{{ $solicitud->cliente }}</td>
                        <td>
                            @if($solicitud->codeudores)
                                <ul style="margin: 0; padding-left: 20px;">
                                    @foreach(explode(',', $solicitud->codeudores) as $codeudor)
                                        <li>{{ trim($codeudor) }}</li>
                                    @endforeach
                                </ul>
                            @else
                        ...
                        <td class="text-bold">{{ number_format($solicitud->importe_solicitud, 2, ',', '.') }} {{ $solicitud->moneda }}</td>
                        <td>{{ $solicitud->nro_cuotas }} ({{ $solicitud->lapso_capital }})</td>
                        ...
                        <td>
                            @php
                                ...
                                if ($solicitud->estado == 1) {
                                    if ($solicitud->tipo_solicitud == 'Nuevo') $estadoText = 'Nuevo';
                                    elseif ($solicitud->tipo_solicitud == 'Reprogramada') $estadoText = 'Reprogramación';
                                    elseif ($solicitud->tipo_solicitud == 'Refinanciada') $estadoText = 'Refinanciada';
                                    else $estadoText = 'Nuevo';
                                } elseif ($solicitud->estado == 2) {
                                    if ($solicitud->tipo_solicitud == 'Nuevo') $estadoText = 'Normal';
                                    elseif ($solicitud->tipo_solicitud == 'Reprogramada') $estadoText = 'Reprogramado';
                                    elseif ($solicitud->tipo_solicitud == 'Refinanciada') $estadoText = 'Refinanciada';
                                ...
                                $estadoClass = '';
                                if ($solicitud->estado == 1) {
                                    if ($solicitud->tipo_solicitud == 'Nuevo') $estadoClass = 'bg-info';
                                    elseif ($solicitud->tipo_solicitud == 'Reprogramada') $estadoClass = 'bg-warning';
                                    elseif ($solicitud->tipo_solicitud == 'Refinanciada') $estadoClass = 'bg-primary';
                                    else $estadoClass = 'bg-info';
                                } elseif ($solicitud->estado == 2) {
                                    if ($solicitud->tipo_solicitud == 'Nuevo') $estadoClass = 'bg-success';
                                    elseif ($solicitud->tipo_solicitud == 'Reprogramada') $estadoClass = 'bg-primary';
                                    elseif ($solicitud->tipo_solicitud == 'Refinanciada') $estadoClass = 'bg-primary';
                            ...
                            @if($solicitud->desembolso == 0)
                                <span class="fas fa-exclamation-circle" style="font-size:0.8rem; color:#ff7c7c; margin-left:5px;" title="IMPORTE POR DESEMBOLSAR">Sin Desembolsar</span>
                            @endif
                        </td>
                    </tr>
                @empty
