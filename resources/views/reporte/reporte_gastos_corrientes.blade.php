    ...
    <div class="container">
            ...
            <tr>
                <td style="padding: 0; text-align: center; vertical-align: middle;">
                    <div style="width: 100%; height: 100%; justify-content: center; align-items: center;">
                        <h1 style="font-size: 20px; margin: 0; padding: 10px 0; text-align: center; color: rgb(0, 0, 0);">LISTADO DE GASTOS CORRIENTES</h1>
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table style="width: 40%;">
            <tr style="padding-top: 5px; padding-bottom: 10px;">
                ...
                <td class="header-left" style="vertical-align: middle;">
                    <p>{{ number_format($total_gastos_corrientes, 2, ',', '.') }}</p>
                </td>
            </tr>
        </table>
        ...
        <table class="data-table">
            ...
            <tbody>
                @foreach($lista_gastos_corrientes as $detalle)
                <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $detalle->id }}</td>
                    <td>{{ $detalle->asesor }}</td>
                    <td>{{ date('d/m/Y', strtotime($detalle->fecha)) }}</td>
                    <td>{{ number_format($detalle->monto_gasto, 2, ',', '.') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
